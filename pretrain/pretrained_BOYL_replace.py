"""BYOL-style text pretraining — Lexical Substitution views.

Giống hệt text_byol.py nhưng dùng lexical-substitution làm augmentation:
...
HF_REPO_ID = "ducanhdinh/jepa_proof_boyl_replace"
...
@dataclass
class BYOLReplacePretrainConfig:
    ...
    output_dir: str = os.path.join(PROJECT_ROOT, "outputs", "text_byol_replace")
    checkpoint_name: str = "text_byol_replace_best.pt"
    latest_checkpoint_name: str = "text_byol_replace_latest.pt"
    loss_plot_name: str = "text_byol_replace_loss.png"
    plot_every: int = 10
...
# ---------------------------------------------------------------------------
# Model — giữ nguyên 100% so với text_byol.py
# ---------------------------------------------------------------------------

class TextBYOLReplace(nn.Module):
    def __init__(self, cfg: BYOLReplacePretrainConfig):
        super().__init__()
...
class BYOLReplacePretrainer:
    def __init__(self, cfg: BYOLReplacePretrainConfig):
        self.cfg = cfg
        set_seed(cfg.data.seed)
        self.device = device_from_config(cfg.device)
        self.model = TextBYOLReplace(cfg).to(self.device)
        ...
        self.start_epoch = 0
        self.global_step = 0
        self.plotter = LossPlotter(self.loss_plot_path(), "BYOL-Replace Pretraining Loss")
        if cfg.resume_from_latest:
            self.load_latest_if_available()

    # ── Train step — giữ nguyên ───────────────────────────────────────────────
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.start_epoch, self.cfg.optim.epochs):
            pbar = tqdm(self.train_loader, desc=f"BYOL-Replace epoch {epoch + 1}", ncols=120)
            for batch in pbar:
    ...
    def push_to_hub(self, repo_id: str = HF_REPO_ID) -> None:
        """Push online encoder + full model weights + model card lên Hugging Face Hub."""
        if not HF_TOKEN:
        ...
        # Full model weights (online_encoder + online_projector + online_predictor
        ...
        api.upload_file(
            ...
            repo_id=repo_id,
            repo_type="model",
            commit_message="Add TextBYOL-Replace full model weights",
        )
        print("   ✓ Model weights đã được push.")

        # Online encoder BERT riêng (dễ dùng lại)
...
def main() -> None:
    trainer = BYOLReplacePretrainer(BYOLReplacePretrainConfig())
    trainer.train()
